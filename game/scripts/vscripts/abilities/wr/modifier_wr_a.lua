...
if IsServer() then
    ...
    function modifier_wr_a:OnIntervalThink()
        if self:GetParent():IsMoving() and not self:GetParent():HasModifier("modifier_wr_e") and not self:GetParent():HasModifier("modifier_silence_lua") and not self:GetParent():HasModifier("modifier_am_r") then
...
function modifier_wr_a:GetModifierMoveSpeedBonus_Percentage(params)
    if self:GetParent():HasModifier("modifier_silence_lua") or self:GetParent():HasModifier("modifier_am_r") then
        return 0
    else
        return 80
    end
