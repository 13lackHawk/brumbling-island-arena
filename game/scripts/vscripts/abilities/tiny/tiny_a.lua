...
function tiny_a:OnSpellStart()
    ...
    local force = 20
    local mod = hero:FindModifier("modifier_tiny_a")
    local sound = { "Arena.Tiny.HitA2", "Arena.Tiny.HitA" }
    local duration = 0.8
    local notBlockedAction = function(target)
        if instanceof(target, Obstacle) and not mod then
            print('wtf')
            hero:AddNewModifier(hero, self, "modifier_tiny_a", { duration = 2.5 })
        end 
    end
    ...
    hero:AreaEffect({
        ...
        isPhysical = true,
        notBlockedAction = notBlockedAction
    })
