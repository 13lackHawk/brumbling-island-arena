...
function IsAbilitySilenced(ability)
    if ability.silenceProtection then
        ...
        for mod, silencelevel in pairs(silenceModifiers) do
            if hero:HasModifier(mod) and ability.silenceProtection <= silencelevel then
                return true
            end
        end
    end
...
function IsFullyCastable(ability, location)
    ...
    return ability:IsFullyCastable() and not IsAbilitySilenced(ability) and passesFilters
end

function Wrappers.NormalAbility(ability, silenceProtection)
