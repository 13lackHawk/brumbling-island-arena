...
function sven_a:OnSpellStart()
    ...
    hero:AreaEffect({
        ...
        action = function(target)
            if instanceof(target, Hero) and not SvenUtil.IsEnraged(hero) then -- Add hardcode check for Jugg having 1 or more swords picked?
                if target.data.range == "ranged" then
                    target:Damage(hero, 2, true)
                else
                    target:Damage(hero, 1, true)
                end
            elseif SvenUtil.IsEnraged(hero) then
                target:Damage(hero, 2, true)
            else
                target:Damage(hero, 1, true)
            end
