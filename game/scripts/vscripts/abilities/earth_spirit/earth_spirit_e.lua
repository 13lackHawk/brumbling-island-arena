...
LinkLuaModifier("modifier_earth_spirit_e_cooldown", "abilities/earth_spirit/modifier_earth_spirit_e_cooldown", LUA_MODIFIER_MOTION_NONE)
    ...
    ESDash(targetRemnant, hero, target, 1400, {
        ...
        arrivalFunction = function()
            if targetRemnant and not targetRemnant.destroyed and not targetRemnant.standingHero and not targetRemnant.falling then
                ...
                if not hero:HasModifier("modifier_earth_spirit_e_cooldown") then
                    print('alo1')
                    hero:AddNewModifier(hero, self, "modifier_earth_spirit_e_cooldown", { duration = 6 })
                    hero:FindAbility("earth_spirit_e"):EndCooldown()
                end
