...
function ursa_a:OnSpellStart()
    ...
    hero:AreaEffect({
        ...
        action = function(victim)
            if instanceof(victim, Hero) and victim:HasModifier("modifier_ursa_e_debuff") then
                hero:Heal(damage)
            ...
            elseif instanceof(victim, Projectile) or instanceof(victim, DistanceCappedProjectile) then
                hero:Heal(1)
                victim:Destroy()
            end
        end,
