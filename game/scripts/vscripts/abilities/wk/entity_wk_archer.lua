    ...
    if self:FindModifier("modifier_wk_skeleton"):GetRemainingTime() <= 0 then
        ...
        ArcProjectile(self.round, {
            ...
            hitParams = {
                ...
                filter = Filters.Area(self.target, 200) + Filters.WrapFilter(
                    function(target)
                        return not instanceof(target, Obstacle)
                    end
                ),
            ...
            hitFunction = function(projectile, hit)
                if hit and not instanceof(hit, Obstacle) then
                    self.hero:AddNewModifier(self.hero, self.ability, "modifier_wk_w_speed", { duration = 1.5 })
